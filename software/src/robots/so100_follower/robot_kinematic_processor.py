    ...
    def action(self, action: RobotAction) -> RobotAction:
        ...
        if self._last_pos is not None:
            ...
            if n > self.max_ee_step_m and n > 0:
                ...
                #logging.warning("EE jump %.3fm > %.3fm", n, self.max_ee_step_m)
...
class InverseKinematicsEEToJoints(RobotActionProcessorStep):
    ...
    def action(self, action: RobotAction) -> RobotAction:
        ...
        position_weight = self.weights["position"] if self.weights is not None else 1.0
        orientation_weight = self.weights["orientation"] if self.weights is not None else 0.01

        q_target = self.kinematics.inverse_kinematics(self.q_curr, t_des, position_weight=position_weight, orientation_weight=orientation_weight)
